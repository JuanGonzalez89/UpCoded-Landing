...
export default function ResultsSection({ dict }: { dict?: { title?: string; items?: readonly { figure: string; unit: string; label: string; source: string }[]; quote?: string; quoteBy?: string } }) {
    ...
    <section className="on-ink bg-ink px-margin-mobile py-24 text-on-ink md:px-margin-desktop lg:py-32">
      <div className="mx-auto max-w-container-max">
        <h2 className="max-w-measure text-headline-lg text-on-ink">
          {dict?.title ?? 'Lo que entregamos y en cuánto tiempo'}
        </h2>
        ...
        <FadeInView delay={200}>
          <figure className="mt-16 border-t border-ink-outline pt-12 lg:mt-20 lg:pt-14">
            <blockquote className="max-w-[58ch] text-headline-md font-normal leading-snug text-on-ink">
              {dict?.quote ?? 'Llevó adelante la creación de la página de onboarding con gran autonomía, manteniendo al equipo informado y coordinando con distintas áreas para lograr un resultado sólido.'}
            </blockquote>
            <figcaption className="mt-6 font-mono text-code-sm text-on-ink-variant">
              {dict?.quoteBy ?? 'Gonzalo Ferragina, Havas Argentina'}
            </figcaption>
          </figure>
        </FadeInView>
      </div>
