...
// Rate limit en memoria por IP. Es best-effort: en serverless cada instancia
// tiene su propio contador, pero frena el abuso básico sin sumar dependencias.
...
/** Una sola línea, para asuntos de email (evita inyección de encabezados). */
...
export async function POST(req: NextRequest) {
  ...
  console.log('[lead] created', { id: lead.id, lang: lead.lang, service: lead.service, stage: lead.stage });
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ${[
                ...
                .map(
                  ([label, value]) => `
                <tr>
                  <td style="padding: 12px 16px 12px 0; border-bottom: 1px solid #3c494d; color: #64748b; width: 140px; font-size: 13px; vertical-align: top;">${esc(label)}</td>
                  <td style="padding: 12px 0; border-bottom: 1px solid #3c494d; color: #e2e8f0; font-size: 13px; vertical-align: top;">${esc(value)}</td>
                </tr>`,
                )
                .join('')}
            </table>
            <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #3c494d;">
              <p style="color: #64748b; font-size: 12px; margin: 0;">Respondé por WhatsApp a ${esc(lead.whatsapp)} o por email a ${esc(lead.email ?? 'upcodednow@gmail.com')}.</p>
            </div>
          </div>
        </div>
      `,
