            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {posts.map((post) => (
                <Link
                  key={post.slug}
                  href={`/blog/${post.slug}`}
                  className="group flex flex-col rounded-lg border border-outline-variant bg-surface-container p-7 transition-[transform,border-color] duration-200 ease-upcoded hover:-translate-y-[3px] hover:border-primary"
                >
                  ...
                  <h2 className="text-headline-md mb-3 text-on-surface transition-colors group-hover:text-primary">
                    {post.title}
                  </h2>
                  <p className="mb-4 flex-grow text-body-sm text-on-surface-variant">
                    {post.description}
                  </p>
                  <span className="mt-auto flex items-center gap-1.5 text-[0.9375rem] font-medium text-primary">
                    Leer artículo
                    <PiArrowRight
                      aria-hidden
                      size={15}
                      className="transition-transform duration-200 ease-upcoded group-hover:translate-x-0.5"
                    />
                  </span>
                </Link>
              ))}
